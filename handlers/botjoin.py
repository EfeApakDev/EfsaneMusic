...
async def addchannel(client, message):
    ...
    try:
        ...
        await message.reply_text(
            "<b>Beni �nce Y�netici Yapmalsnz</b>",
        )
        return
    ...
    try:
        ...
        await message.reply_text(
            ...
            "\n\n Yada Asistan Hesabnz ile Gruba Kendin Ekle </b>",
        )
        return
    await message.reply_text(
            "<b>Asistan zaten bu grupta 🙄</b>",
...
async def rem(USER, message):
    try:
        ...
        await message.reply_text(
            f"<b>Kullanc grubunuzdan ayrlamadm�!."
            "\n\nYada Kendin �karabilirsin</b>",
        )
        return
...
async def rem(USER, message):
    try:
        ...
        await message.reply_text(
            f"<b>Kullanc grubunuzdan ayrlamadm Sel bekliyor olabilir."
...
async def bye(client, message):
    if message.from_user.id in SUDO_USERS:
        left = 0
        failed = 0
        lol = await message.reply("T�m Gruplardan ayrlyorum...")
        async for dialog in USER.iter_dialogs():
            try:
                await USER.leave_chat(dialog.chat.id)
                left = left + 1
                await lol.edit(
                    f"Ayrlyorum... Left: {left} chats. Failed: {failed} chats."
                )
            except:
                failed = failed + 1
                await lol.edit(
